...
func _enter_tree():
	var editor :EditorSettings = get_editor_interface().get_editor_settings()
	print(editor)
#	print(editor.get_property_list())
#	print(editor.get("shortcuts"))
#	editor.add_shortcut()
	for setting in editor.get_property_list():
		print(setting.name)
		if setting.name == "shortcuts":
			print(setting)
